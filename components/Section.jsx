...
export default function Section() {
  ...
  return (
    <div
      className="flex flex-col max-w-full overflow-x-hidden mx-auto"
      id="sectionDiv"
    >
              ...
              {langs.map((Lang, i) => (
                <SwiperSlide key={i} className="relative left-2 ">
                  <div className="h-96  w-72 flex  no-border bg-gradient-to-tr from-blue-100/20 to-[#dedd81]/10 justify-evenly rounded-t-xl">
                    <div
                      className={`w-72 no-border cursor-pointer border-b-4 rounded-xl h-56 object-cover text-center self-end rounded-t-xl duration-500 delay-100  ${active ===
                        i && "h-96"} `}
                    >
                      ...
                      <h2
                        className={`text-2xl  mt-2 font-semibold font-josefin ${active ===
                          i && "text-white"}`}
                      >
                        {Lang.name}
                      </h2>
                      <p
                        className={`para mt-3 font-oswald text-base  px-3   ${active ===
                          i && "h-96"}`}
                      >
                        {Lang.des}
                      </p>
